...
export default {
  ...
  computed: {
    categorySelected() {
      if (this.$route.query.cid == this.item.id && this.$route.query.type == 'background') {
        return true
      }
      return false;
    }
  ...
  methods: {
    selectLine() {
      this.$router.push({name: '图库列表', query: {type: 'background', cid: this.item.id}})
      this.$store.commit('IMAGE_PAGE_TYPE', 'background')
      this.$bus.emit('GET_IMAGE_LIST', Object.assign(this.item, {type: 'background'}))
      this.$bus.emit('ImageSelectChildLine', Object.assign(this.item, {type: 'background'}))
    },
    editProductCat() {
      this.editing = true
    },
    ...
    async edit() {
      ...
      if (!data.code) {
        this.$Modal.remove();
        this.$bus.emit('getBackgroundLines')
        this.editing = false
      }
