...
export default {
  ...
  methods: {
    selectLine() {
      this.$router.push({name: '图库列表', query: {type: 'default', cid: this.item.id}})
      this.$bus.emit('GET_IMAGE_LIST', Object.assign(this.item, {type: 'default'}))
      this.$bus.emit('ImageSelectChildLine', Object.assign(this.item, {type: 'default'}))
    },
    editProductCat() {
      this.editing = true
    },
