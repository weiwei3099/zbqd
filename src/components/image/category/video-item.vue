...
export default {
  ...
  computed: {
    categorySelected() {
      if (this.$route.query.cid == this.item.id && this.$route.query.type == 'default') {
        return true
      }
      return false;
    }
  ...
  methods: {
    selectLine() {
      this.$router.push({name: '图库列表', query: {type: 'video', cid: this.item.id}})
      this.$bus.emit('GET_IMAGE_LIST', Object.assign(this.item, {type: 'video'}))
      this.$bus.emit('ImageSelectChildLine', Object.assign(this.item, {type: 'video'}))
    },
    editProductCat() {
      this.editing = true
    },
    ...
    async edit() {
      let { data } = await this.http('/videoApi/modifyGalleryCategory', {
        data: {
          id: this.item.id,
          title: this.value,
        },
    ...
    async delProductCat() {
      this.$Modal.confirm({
        ...
        loading: true,
        onOk: async () => {
          let { data } = await this.http('/videoApi/delGalleryCategory', {
            data: {
              id: this.item.id
            },
            method: 'POST'
