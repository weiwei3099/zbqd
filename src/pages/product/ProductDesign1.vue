<template>
  <design-page>
    <div slot="header-toolbar">
      ...
      <top-bar></top-bar>
      <Button @click="go('/product/show_design/' + $route.params.productId)" type="text" style="position: fixed; right: 250px; top: 2px;">
...
export default {
  ...
  components: {
    DesignPage, Playground, ModalSelectCategory, ModalSelectImage, ModalSelectBg, Design, TopBar, SelectedPic
  },
  methods: {
    saveDesign() {
      this.scene()
        ...
        other_product: [
          {
            ...
            products: [
              {
                id: 26, is_default: 1, attr: [{width: 500, height: 800, x: 200, y: 100}]
              }
            ]
          }, {
            title: '配件',
            ...
            products: [
              {
                id: 58, is_default: 1, attr: [{width: 20, height: 99, x: 300, y: 300}]
              },
              {
                id: 59, is_default: 0, attr: [{ width: 20, height: 90}]
              }
            ]
          }
        ]
    ...
    selectList(category) {
      ...
      if (category.title == '产品颜色') {
        ...
        const other_product = this.$store.state.design.products.other_product.find((item)=>{
    ...
    async getMainProduct1() {
      ...
      for (let pic of this.main_product.attr) {
        pic.status = 1
      }

      this.list = {
        title: '产品颜色',
        cid: this.main_product.id,
        status: 1,
        products: this.main_product.attr
      }
      this.$store.commit('MAIN_PRODUCT', this.main_product)
      this.$refs.playground.addElement({
        pic: this.list.products[0],
        categoryName: '产品颜色'
      }, {}, true)
...
.select-lines {
  ...
  .lines-row {
    ...
    position: absolute;
    span {
      display: inline-block; background: #f0f0f0; padding: 2px 5px; border-radius: 2px; cursor: pointer; margin: 1px;
    }
  }
  .ivu-tag {
    width: 66px;
...
.steps{
  position: absolute; right: 250px; top: 50px;
