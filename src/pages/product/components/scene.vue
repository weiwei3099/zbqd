  ...
  methods: {
    setZone() {
      ...
      for (let c in this.elements) {
        ...
        if (this.elements[c].copy) {
          const copyAttr = this.$store.getters.GET_OTHER_PRODUCT_COPY_ATTR({
            title: this.elements[c].categoryName,
            product_id: this.elements[c].product.id
          })
          console.log('ddd', copyAttr)
          leftArr.push(parseInt(copyAttr.x));
          topArr.push(parseInt(copyAttr.y));
          rightArr.push(parseInt(copyAttr.x) + parseInt(copyAttr.width));
          bottomArr.push(parseInt(copyAttr.y) + parseInt(copyAttr.height));
        }
      }
    ...
    addElement({pic, categoryName}, attr = {}, copy = false) {
      ...
      if (!this.elements[categoryName]) {
        ...
        let attr_id = attr.attr_id ? attr.attr_id : this.attr_id
