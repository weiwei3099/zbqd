	...
	SVG.extend(SVG.Doc, SVG.Nested, {
		...
		panZoom: function(options) {
			...
			var pinchZoomStart = function(ev) {
				...
				zoomInProgress = true
				SVG.on(document, 'touchmove.panZoom', pinchZoom, this, {
					passive: false
				})
				SVG.on(document, 'touchend.panZoom', pinchZoomStop, this, {
					passive: false
				})
			}
			...
			var panStart = function(ev) {
                lastTouches = normalizeEvent(ev)
                if(lastTouches.length > 1)return

				ev.preventDefault()

				this.off('mousedown.panZoom', panStart)
                this.off('touchstart.panZoom', panStart)
                ...
                SVG.on(document, 'touchmove.panZoom', panning, this)
				SVG.on(document, 'touchend.panZoom', panStop, this)
			}
			...
			var panStop = function(ev) {
                ...
                SVG.off(document, 'touchmove.panZoom', panning)
				SVG.off(document, 'touchend.panZoom', panStop)
				this.on('touchstart.panZoom', panStart)
			}
